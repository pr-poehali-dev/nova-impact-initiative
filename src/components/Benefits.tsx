...
export function Benefits() {
  return (
    <section id="benefits" className="content-section relative z-10 py-16 sm:py-24 md:py-32 border-t border-border/30">
      <div className="container">
        <div className="mb-10 sm:mb-16">
          ...
          <h2 className="font-sentient text-3xl sm:text-4xl md:text-5xl leading-tight max-w-xl">
            Каждый инструмент — прямой контакт с{" "}
            <i className="font-light">вашей аудиторией</i>
          </h2>
          <p className="font-mono text-sm text-foreground/50 mt-4 sm:mt-6 max-w-md">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-border/20">
          {benefits.map((b, i) => (
            <div
              key={i}
              className="bg-background p-6 sm:p-8 hover:bg-white/5 transition-colors duration-300 group"
            >
              <div className="text-2xl sm:text-3xl mb-4 sm:mb-5">{b.icon}</div>
              <h3 className="font-sentient text-lg sm:text-xl text-foreground mb-2 sm:mb-3 group-hover:text-primary transition-colors duration-300">
                {b.title}
              </h3>
              <p className="font-mono text-xs sm:text-sm text-foreground/50 leading-relaxed">{b.desc}</p>
            </div>
          ))}
        </div>
      </div>
