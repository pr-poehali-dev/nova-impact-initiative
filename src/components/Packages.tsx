      ...
      <div className="container">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {packages.map((pkg, i) => (
            <div
              key={i}
              className={`border flex flex-col p-8 transition-colors duration-300 ${
                pkg.highlight
            ...
            >
              <div className="mb-6">
                <p className="font-mono text-xs text-foreground/50 mb-3">{pkg.badge}</p>
                <h3 className="font-sentient text-2xl text-foreground mb-2">{pkg.title}</h3>
                <p className="font-mono text-sm text-foreground/50 leading-relaxed">{pkg.desc}</p>
              </div>

              <div className="border-t border-border/20 pt-6 mb-6">
                <div className={`font-sentient text-3xl ${pkg.highlight ? "text-primary" : "text-foreground"}`}>
                  {pkg.price}
                </div>
